...
export class PlayerBuilder {
  ...
  getInventory(player: Player) {
    return (
      player.getComponent(
        "minecraft:inventory"
      ) as Minecraft.EntityInventoryComponent
    ).container;
  }
  /**
  ...
  getEquipment(player: Player) {
    ...
    ) as Minecraft.EntityEquippableComponent;
  }
  /**
  ...
  getDirection(player: Player) {
    ...
    const isNorth =
      Math.Vector3Utils.dot(facing, Math.VECTOR3_FORWARD) > 0 ? true : false;
    const isSouth =
      Math.Vector3Utils.dot(facing, Math.VECTOR3_BACK) > 0 ? true : false;
    const isWest =
      Math.Vector3Utils.dot(facing, Math.VECTOR3_LEFT) > 0 ? true : false;
    const isEast =
      Math.Vector3Utils.dot(facing, Math.VECTOR3_RIGHT) > 0 ? true : false;
    const isUpOrDown =
      facing.y > 1 / 2 ? true : facing.y < -1 / 2 ? true : false;
    return `${isNorth}, ${isSouth}, ${isWest}, ${isEast}, ${isUpOrDown}`;
  }
}
