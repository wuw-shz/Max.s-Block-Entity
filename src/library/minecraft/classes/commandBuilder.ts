...
export class CommandPosition implements CustomArgType {
  ...
  relativeTo(target: Player | Entity | Block, isBlockLoc = false): Vector3 {
    const loc = { x: 0, y: 0, z: 0 };
    const x = this.x + (this.xRelative ? target.location.x : 0);
    const y = this.y + (this.yRelative ? target.location.y : 0);
    const z = this.z + (this.zRelative ? target.location.z : 0);
...
export class CommandBuilder {
  ...
  register(
    ...
    callback: storedRegisterInformation["callback"]
  ): void {
  ...
  parseArgs(comnand: string, args: Array<string>): Map<string, any> {
    ...
    const processArg = (
      ...
      if (type == "int") {
        if (!/^[-+]?(\d+)$/.test(args[idx])) {
          throw RawText.translate("commands.generic.num.invalid").with(
            args[idx]
          );
        }
        ...
        if (def.range) {
          ...
          if (less) {
            throw RawText.translate("commands.generic.wedit:tooSmall")
              .with(val)
              .with(def.range[0]);
          } else if (greater) {
            throw RawText.translate("commands.generic.wedit:tooBig")
              .with(val)
              .with(def.range[1]);
          }
        }
    ...
    const processList = (
      currIdx: number,
      argDefs: commandArgList,
      result: Map<string, any>,
      flagDefs?: Map<string, commandFlag>
    ) => {
      let defIdx = 0;
        ...
        for (const sub of unnamedSubs) {
          try {
            ...
            invalidFlags.forEach((f, i) => {
              if (
                sub.args
                  .map((argDef) => ("flag" in argDef ? argDef.flag : null))
                  .includes(f)
              ) {
                result.set(f, true);
                invalidFlags[i] = "";
      ...
      if (invalidFlags.length != 0) {
        throw RawText.translate("commands.generic.wedit:invalidFlag").with(
          invalidFlags[0]
        );
      }
  ...
  callCommand(
    ...
    const getCommand = Command.getAllRegistation().some(
      (element) =>
        element.name === command ||
        (element.aliases && element.aliases.includes(command))
    );
    if (!getCommand) {
      throw RawText.translate("commands.generic.unknown")
        .with(`${command}`)
        .printError(player);
    }
    ...
    for (const element of Command.getAllRegistation()) {
      ...
      try {
        if (
          element.permission &&
          !playerHandler.hasPermission(player, element.permission)
        ) {
          throw RawText.translate("commands.generic.wedit:noPermission");
        }
        result = element.callback(
          player,
          msg,
          Command.parseArgs(command, args)
        );
      } catch (e) {
        if (e.isSyntaxError) {
